...
import Notificacoes from "../schema/NotificationSchema";
  ...
  async delete(req, res) {
    ...
    if (isBefore(dataSub, new Date())) {
      return res.status(401).json({
        mensagem: "O horario limite para cancelar o agendamento já apssou",
      });
    }
    ...
    await Mail.sendMail({
      ...
      context: {
        ...
        date: format(agendamento.date, "'dia' dd  'de' MMMM', às ' H:mm'h'", 
        {locale: pt,}
        ),
      },
    }).catch(console.error);
